import { Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, Button, Box, Text, FormControl, FormLabel, Input, FormErrorMessage, useDisclosure } from '@chakra-ui/react';
...
const ModalUpdateContact = () => {
    const { isOpenModalUpdateContact, setIsOpenModalUpdateContact, loadingAttContact, updateContact } = useContext(ContactContext);
    ...
    return (
        ...
        <Modal isOpen={isOpenModalUpdateContact} onClose={handleClose}>
            <ModalOverlay />
            <ModalContent backgroundColor={"white.50"} h={"450px"} borderRadius={"5px"} marginInline={"30px"} borderLeft={"5px solid"} borderColor={"green.100"}>
                ...
                <ModalBody display={"flex"} justifyContent={"center"} flexDirection={"column"}>
                    <form onSubmit={handleSubmit(submit)}>
                        <Box h={"100%"} flexDir={"column"} display={"flex"} gap={"20px"} justifyContent={"space-around"}>
                            ...
                            <Button type="submit" h={{ base: "80px", md: "30px" }} fontSize={{ base: "2xl", md: "xl" }} backgroundColor={"green.50"} color={"green.200"} border={"1px solid"} borderColor={"green.200"} marginTop={"10px"} disabled={loadingAttContact}>
