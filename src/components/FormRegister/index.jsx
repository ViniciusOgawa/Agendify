...
const FormRegister = () => {
    ...
    return (
        <Flex backgroundColor={"white.50"} w={{ base: "1200px", md: "800px" }} h={{ base: "1250px", md: "500px" }} borderRadius={"5px"} marginInline={"30px"} boxShadow={"rgba(149, 157, 165, 0.2) 0px 8px 24px"} justify={"space-between"} borderLeft={"5px solid"} borderColor={"green.100"}>
            <Flex justifyContent={"center"} w={"100%"} h={"100%"} padding={"20"} flexDirection={"column"}>
                <form onSubmit={handleSubmit(submit)}>
                    <Box height={"100%"} flexDir={"column"} gap={"30px"} display={"flex"}>
                        <Text fontSize={{ base: "3xl", md: "2xl" }} fontWeight={"extrabold"} color={"green.200"}>Cadastro</Text>
                        <Flex justifyContent={"space-between"} flexDirection={{ base: "column", md: "row" }} gap={"30px"}>
                            ...
                            <Flex flexDirection={"column"} gap={"20px"}>
                                <FormControl isRequired isInvalid={errors.phoneNumber ? true : false}>
                                    ...
                                    <Input placeholder="Digite seu telefone..." h={{ base: "80px", md: "30px" }} backgroundColor={"green.50"} color={"green.100"} fontSize={{ base: "2xl", md: "xl" }} fontWeight={"medium"} {...register("phoneNumber")} />
                                ...
                                <FormControl isRequired isInvalid={errors.name ? true : false}>
                                    ...
                                    <Input placeholder="Digite seu nome..." h={{ base: "80px", md: "30px" }} backgroundColor={"green.50"} color={"green.100"} fontSize={{ base: "2xl", md: "xl" }} fontWeight={"medium"} {...register("name")} />
                                    <FormErrorMessage fontSize={{ base: "2xl", md: "lg" }} >{errors.name?.message}</FormErrorMessage>
                                </FormControl>
                            </Flex>
                        </Flex>
                        <Button type="submit" h={{ base: "80px", md: "30px" }} fontSize={{ base: "2xl", md: "xl" }} backgroundColor={"green.50"} color={"green.200"} border={"1px solid"} borderColor={"green.200"} marginTop={"10px"} disabled={loadingRegister}>
                            {loadingRegister ? "Cadastrando..." : "Cadastrar"}
                        </Button>
                    </Box>
                </form>
                <Flex gap={"10px"} marginTop={"20px"} >
                    <Text fontSize={{ base: "2xl", md: "lg" }} >Já possui conta?</Text>
                    <ChakraLink as={RouterLink} to="/login" textDecoration="underline" fontSize={{ base: "2xl", md: "lg" }}>Entre</ChakraLink>
                </Flex>
            </Flex>
        </Flex>
    )
